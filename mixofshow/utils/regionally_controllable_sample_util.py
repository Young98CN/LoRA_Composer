...
def register_region_aware_attention(model, layerwise_embedding=False):
    ...
    def get_new_forward(cross_attention_idx):
        ...
        def region_rewrite(self, hidden_states, query, region_list, height, width):
            ...
            def get_region_mask(region_list, feat_height, feat_width):  # 生成mask
            ...
            new_hidden_state[:, region_mask == 0, :] = hidden_states[:, region_mask == 0, :]  # mask为0的地方用原始的latent

            replace_ratio = 1.0  # TODO 替换的比例是否可以修改
            new_hidden_state[:, region_mask != 0, :] = (1 - replace_ratio) * hidden_states[:, region_mask != 0, :]  # mask为1的地方进行修改（这里直接置0）
            for region in region_list:
                region_key, region_value, region_box = region  # 区域lora token映射得到的key，value，定义的bbox [start_h, start_w, end_h, end_w]
                ...
                attention_region = einsum('b h w c, b n c -> b h w n', query[:, start_h:end_h, start_w:end_w, :],
                                          region_key) * self.scale  # 交互latent(query)和token emb(key)
                ...
                new_hidden_state[:, start_h:end_h, start_w:end_w, :] += \
                    ...
                    ).to(query.device))  # region mask还能够用于控制 region prompt生成的latent的比例
        ...
        def new_forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attention_kwargs):
            ...
            if is_cross:
                region_list = []
                # encode_region_prompt_pplus函数定义的region_list
                for region in cross_attention_kwargs['region_list']:
                    if layerwise_embedding:
                        # 这里的region[0]=torch.cat([region_neg_embeds, region_embeds])
                        # [2, 16, 77, 768],77是最大的token长度
                        region_key = self.to_k(region[0][:, cross_attention_idx, ...])  # 将tokenize以后的prompt经过linear得到K
                    ...
                    region_list.append((region_key, region_value, region[1]))  # region[1]是经过归一化后的bbox [start_h, start_w, end_h, end_w]
    ...
    def change_forward(unet, cross_attention_idx):  # omit proceesor in new diffusers
        for name, layer in unet.named_children():
            if layer.__class__.__name__ == 'CrossAttention':
                bound_method = get_new_forward(cross_attention_idx).__get__(layer, layer.__class__)  # 返回new_forward方法，get可能会实例化方法？，不带get会报错丢失第一个参数（可能把第一个参数当做self传入了）
                setattr(layer, 'forward', bound_method)  # 将修改forward的方法存入属性
                if name == 'attn2':
                    cross_attention_idx += 1
            else:
                cross_attention_idx = change_forward(layer, cross_attention_idx)  # 等于continue
...
def encode_region_prompt_pplus(self,
    ...
    if prompt_embeds is None:
        ...
        context_prompt = bind_concept_prompt([context_prompt], new_concept_cfg)  # 将new_concept_cfg中的自定义的token_name替换到prompt中
        context_prompt_input_ids = self.tokenizer(
            context_prompt, # list,len=16
            padding='max_length',
            max_length=self.tokenizer.model_max_length,
            truncation=True,
            return_tensors='pt',
        ).input_ids  # [16,77] 16个句子，最长77个token

        prompt_embeds = self.text_encoder(context_prompt_input_ids.to(device), attention_mask=None)[0]  # [16, 77, 768]
        ...
        for idx, region in enumerate(region_list):
            ...
            region_prompt = bind_concept_prompt([region_prompt], new_concept_cfg)  # 将定义的人名换成cfg中的名字<newxxx>
...
def Regionally_T2IAdaptor_Sample(self,
    ...
    height = height or self.unet.config.sample_size * self.vae_scale_factor  # 这里的尺寸是由输入t2i的pose或sketch图像的大小定义的
        ...
        for k in keys:
            if keypose_adapter_state is not None:
                ...
                spatial_adaptor_weight = keypose_adaptor_weight * torch.ones(*feat_keypose.shape[2:]).to(
                    feat_keypose.dtype).to(feat_keypose.device)  # 定义adapter的权重初始化为1

                if region_keypose_adaptor_weight != '':  # 对特定区域进行权重赋值
            ...
            if sketch_adapter_state is not None:
                ...
                spatial_adaptor_weight = sketch_adaptor_weight * torch.ones(*feat_sketch.shape[2:]).to(
                    feat_sketch.dtype).to(feat_sketch.device)  # 定义adapter的权重初始化为1

                if region_sketch_adaptor_weight != '':  # 对特定区域进行权重赋值
            ...
            adapter_state[k] = torch.cat([feat_keypose + feat_sketch] * 2, dim=0)  # 将多个adapter合并
