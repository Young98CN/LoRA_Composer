...
class EDLoRA(Stable_Diffusion):
                    ...
                    for child_name, child_module in module.named_modules():
                        if child_module.__class__.__name__ == 'Linear':
                            lora_module = LoRALinearLayer(name + '.' + child_name + '_lora', child_module,
                                                          **text_encoder_cfg['lora_cfg'])  # 重新构建了forward过程
    ...
    def init_new_concept(self, new_concept_token, initializer_token):
        ...
        for idx, concept_name in enumerate(new_concept_token):
            ...
            ]  # 每一层decoder都有初始化的token，如第一层对应的是<new0>,<new16>
            ...
            for layer_id, token_id in enumerate(new_token_ids):
                ...
                if init_token_layer.startswith('<rand'):
                    sigma_val = float(re.findall(r'<rand-(.*)>', init_token_layer)[0])  # 获取初始化的比例
                    token_embeds[token_id] = torch.randn_like(token_embeds[0]) * sigma_val  # text emb初始化范围为(0~0.013)
    ...
    def load_delta_state_dict(self, delta_state_dict):  # NOTE 加载LoRA权重到base model中
        # load embedding
        logger = get_root_logger()
        ...
        if 'text_encoder' in delta_state_dict and len(delta_state_dict['text_encoder']) != 0:
            ...
            if hasattr(self, 'text_encoder_lora') and len(load_keys) == 2 * len(self.text_encoder_lora):  # 因为LoRA里有up和down两个模块
    ...
    @torch.no_grad()
    def sample(self, prompt, latents=None, use_negative_prompt=False, num_inference_steps=50, guidance_scale=7.5, seed=None):
        ...
        images = StableDiffusion_PPlus_Sample(
            ...
            generator=torch.Generator().manual_seed(seed) if seed is not None else None).images
        return images
