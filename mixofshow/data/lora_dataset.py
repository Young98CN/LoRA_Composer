...
class LoraDataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        for concept in concept_list:
            instance_prompt = concept['instance_prompt']  # text inversion的token
            caption_dir = concept.get('caption_dir')

            instance_prompt = self.process_text(instance_prompt, replace_mapping)  # 这里为啥要加两个token？

            inst_img_path = []
            for x in Path(concept['instance_data_dir']).iterdir():  # img目录
