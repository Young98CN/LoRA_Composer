...
class EDLoraModel(FinetuneModel):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, seed=None):  # validation 调用的函数
        if isinstance(dataloader.dataset, (PromptDataset)):
            ...
            # sample negprompt
            self.opt['val']['sample']['use_negative_prompt'] = True
            self.visual_validation(dataloader, f'{current_iter}_negprompt', tb_logger, save_img, seed)
        else:
