...
class FinetuneModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # define network
        self.net_g = build_network(opt['network_g'])  # 这里会定义lora
        ...
        if load_path is not None:
            ...
            self.load_delta_network(self.net_g, load_path, param_key)  # 读取权重
    ...
    @torch.no_grad()
    def test(self, test_loss=True, seed=None):
    ...
    def visual_validation(self, dataloader, current_iter, tb_logger, save_img, seed):
        ...
        for idx, val_data in enumerate(dataloader):
            self.feed_data(val_data)
            self.test(test_loss=False, seed=random_seed[idx])  # 结果存在self.output里
            visuals = self.get_current_visuals()  # image list 得到输出结果
    ...
    def load_delta_network(self, net, load_path, param_key='params'):
        ...
        net.load_delta_state_dict(load_net)  # 加载LoRA权重到base model中
